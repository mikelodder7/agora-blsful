...
impl<C: BlsSignatureImpl> ProofCommitment<C> {
    ...
    /// This is step 1 in the three-step process.
    ...
    /// This is step 3 in the three-step process.
    pub fn finalize(
        self,
...
pub struct ProofCommitmentSecret<C: BlsSignatureImpl>(
    /// The raw commitment secret value.
...
impl<C: BlsSignatureImpl> TryFrom<&[u8]> for ProofCommitmentSecret<C> {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let bytes = <[u8; 32]>::try_from(value)
            .map_err(|_| BlsError::InvalidInputs("invalid commitment secret bytes".to_string()))?;
        let value = scalar_from_be_bytes::<C, SECRET_KEY_BYTES>(&bytes).map(Self);
        Option::from(value)
            .ok_or_else(|| BlsError::InvalidInputs("invalid commitment secret bytes".to_string()))
    }
}

impl<C: BlsSignatureImpl> ProofCommitmentSecret<C> {
    /// Get the big-endian byte representation of this commitment secret.
    pub fn to_be_bytes(&self) -> [u8; SECRET_KEY_BYTES] {
        scalar_to_be_bytes::<C, SECRET_KEY_BYTES>(self.0)
    }

    /// Get the little-endian byte representation of this commitment secret.
    pub fn to_le_bytes(&self) -> [u8; SECRET_KEY_BYTES] {
        scalar_to_le_bytes::<C, SECRET_KEY_BYTES>(self.0)
    }

    /// Convert a big-endian representation of the commitment secret.
    pub fn from_be_bytes(bytes: &[u8; SECRET_KEY_BYTES]) -> CtOption<Self> {
        scalar_from_be_bytes::<C, SECRET_KEY_BYTES>(bytes).map(Self)
    }

    /// Convert a little-endian representation of the commitment secret.
...
pub struct ProofCommitmentChallenge<C: BlsSignatureImpl>(
    /// The raw commitment challenge value.
    #[serde(serialize_with = "traits::scalar::serialize::<C, _>")]
    #[serde(deserialize_with = "traits::scalar::deserialize::<C, _>")]
    pub <<C as Pairing>::PublicKey as Group>::Scalar,
);
...
impl<C: BlsSignatureImpl> TryFrom<&[u8]> for ProofCommitmentChallenge<C> {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let bytes = <[u8; 32]>::try_from(value).map_err(|_| {
            BlsError::InvalidInputs("invalid commitment challenge bytes".to_string())
        })?;
        let value = scalar_from_be_bytes::<C, SECRET_KEY_BYTES>(&bytes).map(Self);
        Option::from(value).ok_or_else(|| {
            BlsError::InvalidInputs("invalid commitment challenge bytes".to_string())
        })
    }
}
...
impl<C: BlsSignatureImpl> ProofCommitmentChallenge<C> {
    /// Create a new random commitment challenge.
    pub fn new() -> Self {
        Self::random(get_crypto_rng())
    }

    /// Compute a commitment challenge from a hash.
    ...
    /// Compute a random challenge from a cryptographically secure PRNG.
    ...
    /// Get the big-endian byte representation of this challenge.
    pub fn to_be_bytes(&self) -> [u8; SECRET_KEY_BYTES] {
        scalar_to_be_bytes::<C, SECRET_KEY_BYTES>(self.0)
    }

    /// Get the little-endian byte representation of this challenge.
    pub fn to_le_bytes(&self) -> [u8; SECRET_KEY_BYTES] {
        scalar_to_le_bytes::<C, SECRET_KEY_BYTES>(self.0)
    }
    ...
    /// Convert a little-endian representation of the challenge.
    pub fn from_le_bytes(bytes: &[u8; SECRET_KEY_BYTES]) -> CtOption<Self> {
        scalar_from_le_bytes::<C, SECRET_KEY_BYTES>(bytes).map(Self)
    }
}
